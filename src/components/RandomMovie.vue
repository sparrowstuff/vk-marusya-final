<template>
  <article class="random-movie" v-if="film">
    <div class="random-movie__wrapper">
      <div class="random-movie__info-block">
        <div class="random-movie__details">
          <span class="random-movie__rating" :class="filmRatingColor">
            <svg
              class="rating-movie__star"
              width="16"
              height="16"
              viewBox="0 0 16 16"
              fill="none"
            ...
            <span class="random-movie__rating-text">{{ film.tmdbRating }}</span>
          </span>
          <span class="random-movie__release-year">{{ film.releaseYear }}</span>
          <span class="random-movie__genre">{{ film.genres?.[0] }}</span>
          <span class="random-movie__duration">{{ filmDurationToHrs(film.runtime) }}</span>
        </div>
        <h2 class="random-movie__title">{{ film.title }}</h2>
        <p class="random-movie__description">{{ film.plot }}</p>
        <div class="random-movie__actions">
          <button class="random-movie__trailer-btn btn" type="button" @click="showTrailerWindow">
            Трейлер</button
          ><button class="random-movie__about-btn btn" type="button" @click="goToFilm">
            О&nbspфильме</button
          ><button
            class="random-movie__favorite-btn btn btn--circle"
            :class="{ 'random-movie__favorite-btn--clicked': isInBasket }"
            type="button"
            @click="toggleFavorite"
          >
            <svg
              class="random-movie__like-icon"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
            ...
            <svg
              class="random-movie__like-icon-filled"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
          ...
          <button
            class="random-movie__random-btn btn btn--circle"
            type="button"
            @click="showAnotherRandomFilm"
          >
            <svg
              width="24"
              height="24"
    ...
    >
      <button
        class="random-movie__close-trailer-btn btn btn--close"
        type="button"
        @click="closeTrailerWindow"
      >
        <svg
          class="random-movie__close-icon"
          width="24"
          height="24"
          viewBox="0 0 24 24"
          fill="none"
...
onMounted(async () => {
  try {
    const data = await movies.getRandomMovie()
    film.value = data
  } catch (error) {
...
const filmRatingColor = computed(() => {
  ...
  if (rating >= 8.6) return 'random-movie__rating--gold'
  if (rating >= 7.5) return 'random-movie__rating--green'
  if (rating >= 6.3) return 'random-movie__rating--grey'
  if (rating >= 4.2) return 'random-movie__rating--red'
})

const isInBasket = computed(() => {
  if (!film.value?.id) return false
...
const showAnotherRandomFilm = async () => {
  loading.value = true
...
const goToFilm = () => {
  ...
  router.push(`/film/${film.value.id}`)
}
</script>
...
.random-movie {
  ...
  &:has(.random-movie__watch-trailer-window--show) {
    &::before {
      content: '';
      position: fixed;
      top: 0;
    ...
    > *:not(.random-movie__watch-trailer-window) {
      pointer-events: none;
    }
  }
}
